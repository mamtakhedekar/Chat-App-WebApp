...
public class MessageProvider extends ContentProvider {
	...
	private void saveMessages(Cursor messages) {
		...
		try {
			...
			for (int row=1; row<=messages.getCount(); row++) {
				...
				String message = messages.getString(messages.getColumnIndex(ChatContent.Messages.SENDER));
	...
	/*
	 * If we are downloading several messages at a time, it would be better
	 * to do a bulk insertion on the content provider rather than one row at a time.
	 */
	@Override
